...
function setupUI() {
    ...
    const bgRaw = charData.background?.backgroundId || "";
    ...
    ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'].forEach(s => {
        ...
        grid.innerHTML += `
    <div class="attr-card p-3 text-center transition-all hover:brightness-125">
...
document.getElementById('generateBtn').addEventListener('click', () => {
    ...
    const raceObj = typeof charData.requiredRace === 'string' ? JSON.parse(charData.requiredRace) : charData.requiredRace;
    const classes = charData.jobs.map(j => `${j.jobId.toUpperCase()} (${j.level})`).join(' / ');
    const backgroundRaw = charData.background.backgroundId || "Nenhum";
    doc.setFontSize(8);
    const backgroundFormatado = backgroundRaw
        .replace(/_/g, ' ')
        .split(' ')
        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' ');
    doc.text(`${raceObj.name} | ${classes} | ${backgroundFormatado}`, 15, 28);
    ...
    stats.forEach(s => {
        ...
        doc.setFont("helvetica", temSave ? "bold" : "normal");
    ...
    const secoes = [
        { label: "TRAÇOS: ", content: charData.personalityTraits || "---" },
        { label: "VÍNCULOS: ", content: charData.bonds || "---" },
        { label: "HISTÓRIA: ", content: charData.about || "" }
    ];
    ...
    secoes.forEach(item => {
        ...
        doc.setFont("helvetica", "normal");
        const availableWidth = 180 - labelWidth;
        const contentLines = doc.splitTextToSize(item.content, availableWidth);

        doc.text(contentLines, 15 + labelWidth, y);
        ...
        features.forEach(f => {
            ...
            if (!habilidadesUnicas.has(f.feat.name) && lvlReq <= nivel) {
                ...
                if (isCompact) {
                    ...
                    const lines = doc.splitTextToSize(resumo, 185 - larguraT);
    ...
    if (charData.equipment && charData.equipment.length > 0) {
        ...
        doc.setTextColor(80);
        doc.setFontSize(7);
        const itens = charData.equipment.map(e => `${e.name} (x${e.amount})`).join('  |  ');
        const equipS = doc.splitTextToSize(itens, 180);
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Página ${i} de ${pageCount} | Roll2Paper Core`, 105, 292, { align: 'center' });
    }
